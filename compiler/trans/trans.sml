...
struct
  ...
  and trans_exp env (A.Var(id)) =
    ...
    | trans_exp _ _ = raise IllegalStateException

    (* anything else should be impossible *)

  (* translate the statement *)
  (* trans_stms : Temp.temp Symbol.table -> A.stm list -> Tree.stm list *)
  fun trans_stms env (A.Decl(d)::stms) = trans_stms env stms
    | trans_stms env (A.DeclAssn(d, e)::stms) =
        trans_stms env (A.Assign(d, e)::stms)
    ...
    | trans_stms _ _ = raise IllegalStateException
