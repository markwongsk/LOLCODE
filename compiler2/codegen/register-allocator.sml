...
in
  fun colorGraph g =
    ...
    if Temp.numTemps() < Consts.seoThreshold then (
      ...
      let
        val coloring = CGVHashTable.mkTable (initSize, ElemNotFoundException)
        val max = ref (List.length (Assem.usedSpecialRegs))
...
local
  ...
  fun mapInstr regMap instr =
    ...
    in
      ...
      | Assem.SETF(Assem.TEST(s)) => Assem.SETF(Assem.TEST(allocate' s))
      | Assem.SETF(Assem.CMP(s1, s2)) =>
          Assem.SETF(Assem.CMP(allocate' s1, allocate' s2))
      | _ => instr
    end
in
