...
signature HASH_SET =
  sig
    ...
    val takeElem : set -> item option
    (* Return an element from the hash set if there are any *)
...
functor HashSetFn (Key : HASH_KEY) : HASH_SET =
  ...
  (* Add an item to a set *)
    fun add (tbl as SET{table, elemList, nItems}, item) = let
	  val arr = !table
	  val sz = Array.length arr
	  val h = hash item
	  val indx = index (h, sz)
  ...
  (* Return true if and only if the first set is a subset of the second *)
    fun isSubset (SET{table=tbl1, nItems=n1, ...},
                  s2 as SET{table=tbl2, nItems=n2, ...}) =
          if (!n1 <= !n2)
            then let
  ...
  (* Pull any element from the hash set. *)
    fun takeElem (s as SET{table, elemList, nItems}) =
          case !elemList of
            [] => NONE
          | elem::elems => (
              elemList := elems;
