...
struct
  ...
  local
    fun checkAtMost max wordOpt =
        case wordOpt of
          NONE => NONE
        | SOME word =>
          if Word32.>(word, max) then raise Overflow else SOME word
  in
  (* fromString does not allow leading "-" *)
  fun fromString (str) =
      (* scanString might also raise Overflow *)
      if String.isPrefix "0x" str orelse String.isPrefix "0X" str
      then checkAtMost (Word32.+(TMIN, TMAX))
                       (StringCvt.scanString (Word32.scan StringCvt.HEX) str)
      else checkAtMost TMIN
